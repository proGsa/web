...
#     def __init__(self, repository: IUserRepository, publisher) -> None:
...
class UserService(IUserService):
    ...
    async def add(self, user: User) -> User:
        logger.debug("RPC: user.add %s", user)
        response = await self.rpc.call("user.add", user.model_dump())

        return User(**response)

    async def get_by_id(self, user_id: int) -> User | None:
        logger.debug("RPC: user.get_by_id %d", user_id)
        response = await self.rpc.call("user.get_by_id", {"id": user_id})
        return User(**response) if response else None

    async def get_list(self) -> list[User]:
        logger.debug("RPC: user.get_list")
        items = await self.rpc.call("user.get_list", {})
        return [User(**item) for item in items]

    async def update(self, updated_user: User) -> User:
        logger.debug("RPC: user.update %s", updated_user)
        response = await self.rpc.call("user.update", updated_user.model_dump())
        return User(**response)

    async def delete(self, user_id: int) -> None:
        logger.debug("RPC: user.delete %d", user_id)
        await self.rpc.call("user.delete", {"id": user_id})
...
class AuthService(IAuthService):
    ...
    async def registrate(self, user: User) -> User:
        ...
        logger.debug("RPC: auth.registrate %s", user)
        response = await self.rpc.call("auth.registrate", user.model_dump())


        return User(**response)

    async def authenticate(self, login: str, password: str) -> User | None:
        logger.debug("RPC: auth.authenticate %s", login)
        response = await self.rpc.call("auth.authenticate", {"login": login, "password": password})

        if not response:
            logger.info("Пользователь %s не найден или неверный пароль", login)
            return None
    ...
    @staticmethod
    def create_access_token(user: User) -> str:
        expire = datetime.utcnow() + timedelta(minutes=session_timeout)
        to_encode = {
            "sub": str(user.user_id),
            "login": user.login,
            "is_admin": user.is_admin,
