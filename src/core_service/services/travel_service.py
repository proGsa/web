...
#     def __init__(self, repository: ITravelRepository, publisher) -> None:
...
#             travel = await self.repository.complete(travel_id)

#             if travel:
#                 await self.publisher.publish(
#                     TRAVEL_COMPLETED,
#                     {"travel_id": travel_id, "status": travel.status}
#                 )
#                 logger.info("Путешествие %d успешно завершено", travel_id)
...
#             await self.publisher.publish(
#                 TRAVEL_UPDATED,
#                 {"travel_id": travel_id, "entertainment_ids": entertainment_ids}
#             )
#         except Exception:
...
#             await self.publisher.publish(
#                 TRAVEL_UPDATED,
#                 {"travel_id": travel_id, "user_ids": user_ids}
#             )
#         except Exception:
...
class TravelService(ITravelService):
    ...
    async def get_by_id(self, travel_id: int) -> Travel | None:
        logger.debug("RPC: travel.get_by_id %d", travel_id)
        response = await self.rpc.call("travel.get_by_id", {"id": travel_id})
        return Travel(**response) if response else None

    async def get_all_travels(self) -> list[Travel]:
        logger.debug("RPC: travel.get_all_travels")
        items = await self.rpc.call("travel.get_all_travels", {})
        return [Travel(**item) for item in items]

    async def add(self, travel: Travel) -> Travel:
        logger.debug("RPC: travel.add %s", travel)
        response = await self.rpc.call("travel.add", travel.model_dump())
        return Travel(**response)

    async def update(self, travel: Travel) -> Travel:
        logger.debug("RPC: travel.update %s", travel)
        response = await self.rpc.call("travel.update", travel.model_dump())
        return Travel(**response)

    async def delete(self, travel_id: int) -> None:
        logger.debug("RPC: travel.delete %d", travel_id)
        await self.rpc.call("travel.delete", {"id": travel_id})

    # async def search(self, travel_dict: dict[str, str]) -> list[Travel]:
    #     logger.debug("RPC: travel.search %s", travel_dict)
    #     items = await self.rpc.call("travel.search", travel_dict)
    #     return [Travel(**item) for item in items]

    async def complete(self, travel_id: int) -> Travel | None:
        logger.debug("RPC: travel.complete %d", travel_id)
        response = await self.rpc.call("travel.complete", {"id": travel_id})
        if response:
            return Travel(**response)
        return None

    async def get_users_by_travel(self, travel_id: int) -> list[User]:
        logger.debug("RPC: travel.get_users_by_travel %d", travel_id)
        items = await self.rpc.call("travel.get_users_by_travel", {"id": travel_id})
        return [User(**item) for item in items]

    async def get_entertainments_by_travel(self, travel_id: int) -> list[Entertainment]:
        logger.debug("RPC: travel.get_entertainments_by_travel %d", travel_id)
        items = await self.rpc.call("travel.get_entertainments_by_travel", {"id": travel_id})
        return [Entertainment(**item) for item in items]

    async def get_accommodations_by_travel(self, travel_id: int) -> list[Accommodation]:
        logger.debug("RPC: travel.get_accommodations_by_travel %d", travel_id)
        items = await self.rpc.call("travel.get_accommodations_by_travel", {"id": travel_id})
        return [Accommodation(**item) for item in items]

    async def link_entertainments(self, travel_id: int, entertainment_ids: list[int]) -> None:
        logger.debug("RPC: travel.link_entertainments %d -> %s", travel_id, entertainment_ids)
        await self.rpc.call("travel.link_entertainments", {"id": travel_id, "entertainment_ids": entertainment_ids})

    async def link_users(self, travel_id: int, user_ids: list[int]) -> None:
        logger.debug("RPC: travel.link_users %d -> %s", travel_id, user_ids)
        await self.rpc.call("travel.link_users", {"id": travel_id, "user_ids": user_ids})

    async def link_accommodations(self, travel_id: int, accommodation_ids: list[int]) -> None:
        logger.debug("RPC: travel.link_accommodations %d -> %s", travel_id, accommodation_ids)
        await self.rpc.call("travel.link_accommodations", {"id": travel_id, "accommodation_ids": accommodation_ids})

    async def get_travels_for_user(self, user_id: int, travel_status: str) -> list[Travel]:
        logger.debug("RPC: travel.get_travels_for_user %d status=%s", user_id, travel_status)
        items = await self.rpc.call("travel.get_travels_for_user", {"user_id": user_id, "status": travel_status})
        return [Travel(**item) for item in items]

    async def get_travel_by_route_id(self, route_id: int) -> Travel | None:
        logger.debug("RPC: travel.get_travel_by_route_id %d", route_id)
        response = await self.rpc.call("travel.get_travel_by_route_id", {"route_id": route_id})
        return Travel(**response) if response else None

    async def unlink_entertainment(self, travel_id: int, entertainment_id: int) -> None:
        logger.debug("RPC: travel.unlink_entertainment %d -> %d", travel_id, entertainment_id)
        await self.rpc.call("travel.unlink_entertainment", {"travel_id": travel_id, "entertainment_id": entertainment_id})

    async def unlink_accommodation(self, travel_id: int, accommodation_id: int) -> None:
        logger.debug("RPC: travel.unlink_accommodation %d -> %d", travel_id, accommodation_id)
        await self.rpc.call("travel.unlink_accommodation", {"travel_id": travel_id, "accommodation_id": accommodation_id})