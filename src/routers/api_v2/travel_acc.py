from __future__ import annotations

import logging

from fastapi import APIRouter
from fastapi import Depends
from fastapi import HTTPException
from fastapi import status

from schemas.accommodation import AccommodationCreate
from schemas.accommodation import AccommodationResponse
from service_locator import ServiceLocatorV2
from service_locator import get_service_locator_v2


logger = logging.getLogger(__name__)
router = APIRouter(prefix="/travels", tags=["travel-accommodations"])
get_sl_dep = Depends(get_service_locator_v2)


@router.post(
    "/{travel_id}/accommodations",
    response_model=AccommodationResponse,
    status_code=status.HTTP_201_CREATED,
    summary="Добавить размещение к маршруту",
    description="Создает новое размещение и добавляет его к указанному путешествию",
    responses={
        400: {"description": "Неверный запрос"},
        404: {"description": "Путешествие не найдено"},
        500: {"description": "Внутренняя ошибка сервера"},
    },
)
async def add_accommodation_to_travel(
    travel_id: int,
    accommodation: AccommodationCreate,
    service_locator: ServiceLocatorV2 = get_sl_dep,
):
    """
    Создать размещение и добавить его к путешествию
    """
    try:
        result = await service_locator.get_travel_contr().add_accommodation_to_travel(travel_id, accommodation)
        logger.info("Размещение '%s' добавлено к путешествию ID %d", accommodation.name, travel_id)
        return result
    except ValueError as e:
        raise HTTPException(status_code=400, detail=str(e))
    except KeyError:
        raise HTTPException(status_code=404, detail="Travel not found")
    except Exception as e:
        logger.error(f"Ошибка при добавлении развлечения к путешествию: {e}", exc_info=True)
        raise HTTPException(status_code=500, detail="Internal server error")


@router.delete(
    "/{travel_id}/accommodations/{accommodation_id}",
    status_code=status.HTTP_204_NO_CONTENT,
    summary="Удалить размещение из путешествия",
    description="Удаляет размещение из указанного путешествия",
    responses={
        404: {"description": "Размещение или путешествие не найдены"},
        500: {"description": "Внутренняя ошибка сервера"},
    },
)
async def delete_accommodation_from_travel(
    travel_id: int,
    accommodation_id: int,
    service_locator: ServiceLocatorV2 = get_sl_dep,
):
    """
    Удалить размещение из путешествия
    """
    try:
        await service_locator.get_travel_contr().delete_accommodation_from_travel(travel_id, accommodation_id)
        logger.info("Размещение ID %d удалено из путешествия ID %d", accommodation_id, travel_id)
    except KeyError:
        raise HTTPException(status_code=404, detail="Travel or accommodation not found")
    except Exception as e:
        logger.error(f"Ошибка при удалении развлечения из путешествия: {e}", exc_info=True)
        raise HTTPException(status_code=500, detail="Internal server error")
