    ...
    async def get_route_parts(self, travel_id: int) -> list[dict[str, Any]]:
        query = text("""
            SELECT 
                ...
                r.end_time,
                r.type
            FROM travel_db.route r
            JOIN travel_db.directory_route dr ON r.d_route_id = dr.id
            JOIN travel_db.city dc ON dr.departure_city = dc.city_id
            JOIN travel_db.city ac ON dr.arrival_city = ac.city_id
